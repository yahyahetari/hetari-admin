...
export default function Orders() {
    ...
    const ordersPerPage = 10; // عدد الطلبات في كل صفحة
    ...
    useEffect(() => {
        // تحميل قائمة الطلبات التي تم النقر عليها من Local Storage عند تحميل الصفحة
        const savedClickedOrders = JSON.parse(localStorage.getItem('clickedOrders')) || [];
        setClickedOrders(savedClickedOrders);

        // تحميل رقم الصفحة المحفوظة من Local Storage
    ...
    const handleOrderClick = (orderId) => {
        const updatedClickedOrders = [...clickedOrders, orderId];
        setClickedOrders(updatedClickedOrders);
        // حفظ قائمة الطلبات التي تم النقر عليها في Local Storage
        localStorage.setItem('clickedOrders', JSON.stringify(updatedClickedOrders));

        // حفظ رقم الصفحة الحالية في Local Storage
    ...
    // تحديد الطلبات المعروضة في الصفحة الحالية
    ...
    {currentOrders.map(order => (
        <tr key={order._id} className="hover:bg-glass items-center justify-center">
            <td className="px-5 py-5 border-gray-200 text-lg flex items-center whitespace-nowrap overflow-hidden text-ellipsis">
                ...
                {!clickedOrders.includes(order._id) && (
