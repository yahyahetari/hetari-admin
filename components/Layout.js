...
export default function Layout({ children }) {
  ...
  if (loading) {
    return (
        <div className="flex justify-center items-center h-screen bg-glass">
            <Loader />
        </div>
    );
}
  ...
  return (
    <div className="text-white bg-bg-img bg-cover min-h-screen bg-glass overflow-hidden">
      ...
      <div className="flex">
        ...
        <main className={`flex-grow m-2 w-54 p-4 ${isMobileOrLess ? 'ml-2 h-[500px]' : 'ml-60 '} rounded-lg bg-glass h-[600px] overflow-y-auto w-54 custom-scrollbar`}>
          {children}
        </main>
      </div>
    </div>
