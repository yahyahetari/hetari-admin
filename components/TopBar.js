...
export default function TopBar() {
  ...
  useEffect(() => {
    const handleRouteChange = () => {
      setDropdownMenu(false); // Close the dropdown menu on route change
    };
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const clickedOrders = JSON.parse(localStorage.getItem('clickedOrders')) || [];
        const newOrders = orders.filter(order => !clickedOrders.includes(order._id));
    ...
    <div className="sticky top-0 z-20 w-full flex items-center px-4 py-2 bg-blue-2 shadow-xl">
      ...
      <div className="lg:hidden relative ml-auto flex gap-4 items-center">
        <div className="relative">
          ...
          {hasNewOrders && (
            <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-xs font-bold rounded-full h-3 w-3 flex items-center justify-center"></span>
          )}
        </div>
        {dropdownMenu && (
